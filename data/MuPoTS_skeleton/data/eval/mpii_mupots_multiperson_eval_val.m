...
%% load prdictions
preds_2d_kpt = load(sprintf('%s/preds_2d_kpt_mupots.mat', eval_path));
preds_3d_kpt = load(sprintf('%s/preds_3d_kpt_mupots.mat', eval_path));

%val_list = {9, 12, 14, 15, 16, 17};

for ts = 1:20 % i = 1:length(val_list) %1:20
    %ts = val_list{i};
...
%save([results_output_path filesep eval_path out_prefix 'multiperson_3dhp_evaluation.mat'], 'sequencewise_per_joint_error' );

[seq_table] = mpii_evaluate_multiperson_errors(sequencewise_per_joint_error );%fullfile(net_base, net_path{n,1}));
out_file = [results_output_path filesep eval_path out_prefix 'multiperson_3dhp_evaluation'];
writetable(cell2table(seq_table), [out_file '_sequencewise.csv']);
writetable(cell2table(seq_table(:,end)), [results_output_path filesep eval_path '/pck.csv']);
  
%[seq_table] = mpii_evaluate_multiperson_errors_visibility_mask(sequencewise_per_joint_error , sequencewise_visibility_mask);
%out_file = [results_output_path filesep eval_path [out_prefix 'visible_joints_'] 'multiperson_3dhp_evaluation'];
%writetable(cell2table(seq_table), [out_file '_sequencewise.csv']);

%[seq_table] = mpii_evaluate_multiperson_errors_visibility_mask(sequencewise_per_joint_error , sequencewise_occlusion_mask);
%out_file = [results_output_path filesep eval_path [out_prefix 'occluded_joints_'] 'multiperson_3dhp_evaluation'];
%writetable(cell2table(seq_table), [out_file '_sequencewise.csv']);
